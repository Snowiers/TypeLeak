...
def run(host: str, port: int, sample_rate: int, block_size: int,
    ...
    try:
        with sd.InputStream(samplerate=sample_rate, channels=channels,
            ...
            # NOTE: deliberately NOT sd.sleep(10**9) here -- that's a single
            # long blocking call into PortAudio's C layer, which can prevent
            # Python from noticing a pending Ctrl+C until the call returns
            # (i.e. never, in practice). Looping many short sleeps instead
            # gives the interpreter frequent chances to see the interrupt
            # and actually stop promptly.
            while True:
                time.sleep(0.1)
