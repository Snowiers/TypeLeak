...
def main():
    ...
    def on_event(event):
        ...
        print(f"  [event via network] t~{event['sample_index']/config.SAMPLE_RATE:.3f}s "
              f"zone={event['predicted_zone']} exposure={event['exposure_score']:.1f}")
    ...
    def run_server():
        connected_evt.set()  # signal "about to accept" (server.stream() blocks on accept() right after)
        server.stream(pipeline._handle_chunk)
    ...
    time.sleep(0.5)  # let any final pending onsets flush through
